...
class ReportController extends Controller
{
    ...
    {
        if($user->tokenCan(Permission::CAN_ALL) || $user->tokenCan(Permission::CAN_REPORT_DAILY_REPORT)) {
    ...
    {
        if($user->tokenCan(Permission::CAN_ALL) || $user->tokenCan(Permission::CAN_REPORT_Monthly_REPORT)) {
            $user_list = DB::table('users')
                ->join('notifications', 'users.id', '=', 'notifications.user_id')
                ->where('users.active', '=', 1)
                ->where('notifications.status', '=', 1)
    ...
    {
        if($user->tokenCan(Permission::CAN_ALL) || $user->tokenCan(Permission::CAN_REPORT_Weekly_REPORT)) {
            $user_list = DB::table('users')
                ->join('notifications', 'users.id', '=', 'notifications.user_id')
                ->where('users.active', '=', 1)
                ->where('notifications.status', '=', 1)
